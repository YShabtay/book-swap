...
function SelectField({ icon: Icon, value, onChange, disabled, title, children }) {
  return (
    <div className="relative flex-1 min-w-[9.5rem]" title={title}>
      <Icon
        size={16}
      ...
      <select
        ...
        onChange={onChange}
        disabled={disabled}
        className="w-full appearance-none rounded-xl border border-slate-200 bg-white py-2.5 text-sm font-medium text-slate-700 shadow-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:opacity-50 rtl:pr-9 rtl:pl-3 ltr:pl-9 ltr:pr-3"
      >
        {children}
      </select>
    </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 text-sm font-medium text-slate-800 placeholder:text-slate-400 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-2 focus:ring-indigo-100 rtl:pr-10 rtl:pl-3 ltr:pl-10 ltr:pr-3"
          />
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2.5">
          ...
          {hasActiveFilters && (
            <button
              type="button"
              onClick={clearAll}
              className="flex items-center gap-1 rounded-xl border border-transparent px-3 py-2.5 text-sm font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-700"
            >
              <X size={15} />
              {t('clearFilters')}
            </button>
